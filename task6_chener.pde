...
void collision() {
  for (int a=0; a<laserNum; a=a+1) { 
    for (int f=0; f<asteroidNum; f=f+1) {
      if (dist(laserX[a], laserY[a], asteroidX[f], asteroidY[f])<20) {
        initializeAsteroid(f);
        initializeLaser(a);
        //  if (asteroidX[f]<400 && asteroidX[f]>0 && asteroidY[f]<400 && asteroidY[f]>0) {
        score=score+1;
      }
    }
  }
